...
const StockInModal = ({ isOpen, onClose, onSave, stockItems = [], initialValues }) => {
  ...
  // Tedarikçi listesi API'den gelecek
  const [suppliers, setSuppliers] = useState([]);
  const [supplierLoading, setSupplierLoading] = useState(false);
  const [showSupplierModal, setShowSupplierModal] = useState(false);
  // Tedarikçi listesini API'den çek
  const fetchSuppliers = async () => {
    ...
    try {
      ...
      // response.data dizi ise direkt ata, değilse .items veya .data bak
      let supplierData = [];
  ...
  return (
    <>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${errors.itemId ? 'border-red-500' : 'border-gray-300'}`}
                  >
                    ...
                    <div>
                      ...
                      >
                        ...
                        {supplierLoading ? (
                          ...
                          suppliers.map(sup => (
                            <option key={sup.id || sup.companyName || sup.name} value={sup.companyName || sup.name}>{sup.companyName || sup.name}</option>
                          ))
                        )}
                      </select>
                    </div>
      ...
      {/* Tedarikçi Ekle Modalı: form'un DIŞINDA! */}
      {showSupplierModal && (
        <SupplierAddModal
